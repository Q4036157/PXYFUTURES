...
from app.market.ma_engine import Bar, CrossType, MovingAverageConfig, Trend, _classify, calculate_signal


def test_long_average_order_does_not_change_zone() -> None:
    """M3/M4 谁在上不重要，取上下界后得到相同的价格区域。"""
    assert _classify(CrossType.GOLDEN, 90, 100, 120) == "反弹"
    assert _classify(CrossType.GOLDEN, 90, 120, 100) == "反弹"
    assert _classify(CrossType.DEATH, 110, 100, 120) == "震荡"
    assert _classify(CrossType.DEATH, 110, 120, 100) == "震荡"
...
def test_zone_boundaries_are_consolidation() -> None:
    assert _classify(CrossType.GOLDEN, 100, 100, 120) == "震荡"
    assert _classify(CrossType.DEATH, 120, 100, 120) == "震荡"
